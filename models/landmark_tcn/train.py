"""Train landmark TCN (low-compute third model)."""
...
def main() -> None:
    ...
    train_ds = LandmarkDataset(
        train_df["abs_path"].tolist(), train_df["y"].tolist(), cache, args.num_frames, True
    )
    val_ds = LandmarkDataset(
        val_df["abs_path"].tolist(), val_df["y"].tolist(), cache, args.num_frames, False
    )
    ...
    print(f"LandmarkTCN params={n_params:,} device={device}")
    ...
    out_dir = CHECKPOINT_DIR / "landmark_tcn"
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if va_acc >= best:
            best = va_acc
            torch.save(
                {"model": model.state_dict(), "args": vars(args), "best_val_acc": best, "num_classes": len(w2i)},
                out_dir / "best.pt",
            )
