...
def main() -> None:
    ...
    set_seed(args.seed)
    device = torch.device("cpu") if args.cpu else configure_l40s()
    ...
    for name in args.models:
        ...
        if name in ("landmark_tcn", "mediapipe_transformer"):
            ...
            # prefer frames stored in meta
            meta_frames = args.num_frames
            if ckpt.exists():
                blob = torch.load(ckpt, map_location="cpu", weights_only=False)
                meta_frames = int(blob.get("meta", {}).get("num_frames", args.num_frames))
                cache = CACHE_DIR / f"landmarks_T{meta_frames}"
                num_classes = int(blob["meta"]["num_classes"])
                if name == "landmark_tcn":
                    ...
                    model = LandmarkTransformer(
                        FEAT_DIM,
                        num_classes,
                        d_model=int(blob["meta"].get("d_model", 256)),
                        max_len=meta_frames,
                    )
            ...
            loader = DataLoader(ds, batch_size=min(8, args.batch_size), shuffle=False, collate_fn=collate_fn)
