"""
L40S-optimized training for all three ISL models.

Presets tuned for NVIDIA L40S (~48GB): large batches, TF32, AMP (bf16/fp16),
many DataLoader workers, pin_memory. Saves best val weights and final test metrics
under models/_weights/<model_name>/.
"""
...
PRESETS = {
    ...
    "videomae_finetune": {
        ...
        "freeze_backbone": False,  # L40S can full fine-tune
    },
}
...
def train_videomae(train_df, val_df, test_df, labels, args, device):
    ...
    if args.freeze_backbone:
        ...
        print("[videomae] frozen backbone")
    else:
        print("[videomae] full fine-tune")
...
def parse_args():
    ap = argparse.ArgumentParser(description="Train ISL models with L40S presets")
    ap.add_argument(
        "--models",
        nargs="+",
...
def main() -> None:
    cli = parse_args()
    set_seed(cli.seed)
    device = torch.device("cpu") if cli.cpu else configure_l40s()
